...
let currentOptimizeMode = 'textClarity';
...
async function initAICapabilities() {
    ...
    try {
        const promptApiSupported = !!(self.ai && self.ai.languageModel);
        if (!promptApiSupported) {
            console.warn('AI API is not available - falling back to background simplifier');
        }
        ...
        currentOptimizeMode = optimizeFor;
        ...
        switch (request.action) {
            case "simplify":
                try {
                    ...
                    const promptApiReady = !!promptSession;
                    if (!promptApiReady) {
                        console.warn('Prompt API not available - using background fallback');
                    }
                ...
                for (let chunk of chunks) {
                    ...
                    try {
                        ...
                        // Send the chunkText either via Prompt API or fallback service
                        let simplifiedText = '';
                        if (promptSession) {
                            let attempts = 0;
                            ...
                            simplifiedText = await simplifyChunkViaBackground(chunkText);
                        }
...
async function simplifyChunkViaBackground(text) {
    return new Promise((resolve, reject) => {
        chrome.runtime.sendMessage({
            action: 'SIMPLIFY_CHUNK',
            text,
            optimizeMode: currentOptimizeMode,

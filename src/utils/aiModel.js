...
import { GEMINI_API_KEY } from "../config/ai.js";
...
{"tabId":"1181814502","title":"Devpost","url":"https://devpost.com/submit-to/25987-google-chrome-built-in-ai-challenge-2025/manage/submissions/798906-TabPilot/project_details/edit","description":"","textSnippet":""},
...
{"tabId":"1181814438","title":"TabPilot - Presentation","url":"https://www.canva.com/design/DAG3Mi53qBk/Nbk6mSZ_bLQbENzr0HVXzw/edit","description":"","textSnippet":""}
]
...
"summary": "Devpost submissions and presentations for Chrome extensions and AI challenges, including Mochi, Orma, and TabPilot projects."
},
...
async function getSettings() {
  return new Promise((resolve) => {
    chrome.storage.sync.get("TabPilotSettings", (result) => {
      ...
      resolve({ ...defaultSettings, ...result.TabPilotSettings });
    });
  });
}
...
export async function initializeAIModel(forceReinit = false) {
  try {
    ...
    console.log("🔧 INITIALIZING Gemini (GoogleGenAI) MODEL");
    // const apiKey = settings.apiKey || process.env.GEMINI_API_KEY;
    const settings = await getSettings();
    const apiKey = GEMINI_API_KEY;
    if (!apiKey) {
      throw new Error("Gemini API key not found (in settings or env)");
    }
    ...
    model = await aiClient.models;

    
    console.log("✅ GenerativeModel created successfully!", model);
...
export async function processTabsWithAI(tabsData) {
  try {
    ...
    const response = await model.generateContent({
      model: settings.cloudModel || "gemini-2.5-flash",
      contents: prompt,
      generationConfig: {
    ...
    console.log(`✅ Parsed categories: ${Object.keys(parsed).length}`);
    return parsed;
